...
#include "MotorAxisVectorTestSequence.hpp"
...
int main(int argc, char **argv) {
	...
	MotorAxisVectorTestControlSystem controlSys(dt);
	
	// Create and initialize a safety system
	MotorAxisVectorTestSafetyProperties properties(controlSys, td);
	SafetySystem safetySys(properties, td);
	//controlSys.timedomain.registerSafetyEvent(safetySys, properties.seRun);
	
	/*auto& sequencer = Sequencer::instance();
	MotorAxisVectorTestSequence mainSequence("Main Sequence", sequencer, safetySys, properties, controlSys, 3.14/10);
