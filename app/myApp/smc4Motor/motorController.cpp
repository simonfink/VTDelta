...
int main(int argc, char **argv) {
	...
	MotorControlSystem controlSys(dt);
	
	// Create and initialize a safety system
	MotorSafetyProperties properties(controlSys, dt);
	SafetySystem safetySys(properties, dt);
	controlSys.timedomain.registerSafetyEvent(safetySys, properties.doEmergency);
	
	auto& sequencer = Sequencer::instance();
	MotorSequence mainSequence("Main Sequence", sequencer, safetySys, properties, controlSys, 3.14/10);
