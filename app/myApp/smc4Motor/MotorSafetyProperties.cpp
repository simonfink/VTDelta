...
MotorSafetyProperties::MotorSafetyProperties(MotorControlSystem& controlSys, double dt) : 
	...
	enable = hal.getLogicOutput("ledBlue");
	isEmergency = hal.getLogicOutput("ledRed");
	
	criticalOutputs = { enable , isEmergency};
	
	// ############ Define critical inputs ############
	emergency = hal.getLogicInput("buttonRed");
	ready = hal.getLogicInput("buttonBlue");
	...
	slOff			.setOutputActions( { set(enable, false), set(isEmergency, false )} );;
	slEmergency		.setOutputActions( { set(enable, false), set(isEmergency, true )} );;
	slSystemOn		.setOutputActions( { set(enable, false), set(isEmergency, false) } );;
	slStartingControl	.setOutputActions( { set(enable, false), set(isEmergency, false) } );;
	slStoppingControl	.setOutputActions( { set(enable, false), set(isEmergency, false) } );;
	slPowerOn		.setOutputActions( { set(enable, true), set(isEmergency, false) } );;
	slMoving		.setOutputActions( { set(enable, true), set(isEmergency, false) } );;
...
MotorSafetyProperties::~MotorSafetyProperties() { }