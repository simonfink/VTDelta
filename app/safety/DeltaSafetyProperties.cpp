...
DeltaSafetyProperties::DeltaSafetyProperties(DeltaControlSystem& controlSys, double td) : 
	...
	tdn(td)
	{
	  ...
	  slAutoMoving.setInputActions({check(emergencyStop,false,doEmergency),check(approval, false, doMouseTeaching)});
	  slMouseTeaching.setInputActions({check(emergencyStop,false,doEmergency),ignore(approval)});
	  ...
	  slMouseTeaching.setOutputActions({set(led,true), set(errorLed, false)});
