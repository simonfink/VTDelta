...
ScaraSafetyProperties_force::ScaraSafetyProperties_force() :
...
set_force_control          ("Set Force Control"  ),
...
doSetForce_control             ("doSetForce_control       "),
doStartForce_control           ("doStartForce_control     "),
doStopForce_control            ("doStopForce_control      "),
...

{
	...
	addLevel(set_force_control          );
	addLevel(force_control              );
	...
	ready                      .addEvent(doSetForce_control  ,           set_force_control,          kPublicEvent  );
	...
	set_force_control          .addEvent(doStartForce_control,           force_control,              kPublicEvent  );
	force_control              .addEvent(doStopForce_control,            ready,                      kPublicEvent  );
...
// 	addEventToAllLevelsBetween(resetEmergency, autoParking_shutdown0, doEmergency, emergency,         kPublicEvent );
	...
	set_force_control          .addEvent(doEmergency, emergency,         kPublicEvent  );
	force_control              .addEvent(doEmergency, emergency,         kPublicEvent  );
	...
	force_control              .addEvent(doEmergency_posOutRange,        emergency,                  kPublicEvent  );
	force_control              .addEvent(doEmergency_velOutRange,        emergency,                  kPublicEvent  );
	...
	set_force_control          .setInputActions({ check(approval, false, doEmergency),  check(limitSwitchQ0p, true , doEmergency), check(limitSwitchQ0n, true , doEmergency),        check(limitSwitchQ1p, true , doEmergency), check(limitSwitchQ1n, true, doEmergency),         check(limitSwitchQ2p, true, doEmergency),         check(limitSwitchQ2n, true , doEmergency), check(limitSwitchQ3p, true , doEmergency), check(limitSwitchQ3n, true , doEmergency),       });
	force_control              .setInputActions({ check(approval, false, doEmergency),  check(limitSwitchQ0p, true , doEmergency), check(limitSwitchQ0n, true , doEmergency),        check(limitSwitchQ1p, true , doEmergency), check(limitSwitchQ1n, true, doEmergency),         check(limitSwitchQ2p, true, doEmergency),         check(limitSwitchQ2n, true , doEmergency), check(limitSwitchQ3p, true , doEmergency), check(limitSwitchQ3n, true , doEmergency),       });
	...
	set_force_control          .setOutputActions({ toggle(watchdog    ), set(enable0, false), set(enable1, false), set(enable2, false), set(enable3, false), set(brake0, true ), set(brake1, true ), set(brake2, true ), set(brake3, true ) });
	force_control              .setOutputActions({ toggle(watchdog    ), set(enable0, true ), set(enable1, true ), set(enable2, true ), set(enable3, true ), set(brake0, false), set(brake1, false), set(brake2, false), set(brake3, false) });
	...
	init_value.zero();
 	...
 	controlSys->I_pt1.enable();
	controlSys->I_vel_to_pos.setInitCondition(init_value);
	controlSys->I_vel_to_pos.enable();
	controlSys->I_acc_to_vel.setInitCondition(init_value);
	controlSys->I_acc_to_vel.enable();
	controlSys->SatPos.enable();
...
// 
// 
// 	if(off.getNofActivations() % 2000 == 0) {
// 		std::cout << " Force Sum = " << controlSys->forceSum.getIn(1).getSignal().getValue() << std::endl;
// 		std::cout << " " <<  std::endl;
// 		std::cout << " dynModell = " << controlSys->DynModell.getOut().getSignal().getValue() << std::endl;
// 		std::cout << " " <<  std::endl;
// 		std::cout << " I acc to vel = " << controlSys->I_acc_to_vel.getOut().getSignal().getValue() << std::endl;
// 		std::cout << " " <<  std::endl;
// 		std::cout << " I vel to pos = " << controlSys->I_vel_to_pos.getOut().getSignal().getValue() << std::endl;
// 		std::cout << " " <<  std::endl;
// 		std::cout << " Saturation = " << controlSys->SatPos.getOut().getSignal().getValue() << std::endl;
// 		std::cout << " " <<  std::endl;
// 		std::cout << " Saturation = " << controlSys->positionOrForceControlSwitch.getOut().getSignal().getValue() << std::endl;
// 		std::cout << " " <<  std::endl;
// 	}
 	});
	...
	set_moving.setLevelAction([this](SafetyContext* privateContext) 
	{
		...
		//std::cout << " Fx output " << controlSys->fx.getOut().getSignal().getValue() << std::endl;
	...
	set_force_control.setLevelAction([this](SafetyContext* privateContext) 
	{
		AxisVector x_actual = controlSys->dirKin.getOut().getSignal().getValue();
		controlSys->pathPlannerCS.setInitPos(x_actual);
...
// 		
// 			count = 0;
// 			std::cout << " ForcControl output " << controlSys->forceController.getOut().getSignal().getValue() << std::endl;
// 		}
// 		count++;
// 		std::cout << " Fx output " << controlSys->fx.getOut().getSignal().getValue() << std::endl;
		...
		else
		{
			...
			{
				privateContext->triggerEvent(doStartForce_control);
			}
		}
	});
