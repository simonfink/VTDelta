...
EncSafetyProperties::EncSafetyProperties(EncControlSystem* cs) : 
off("SW off"), 
...
controlSys(cs) {
	...
	on.setLevelAction([this](SafetyContext* privateContext) {
		static int count = 0;
		if(count > 100){
			std::cout << "SS: axis0: " << q0->get() << ", axis1: " << q1->get() << ", axis2: " << q2r->get() << ", axis3: " << q3->get() << std::endl;
			auto q0_cs = controlSys->invIencPos0.getOut().getSignal().getValue();
			auto q1_cs = controlSys->invIencPos1.getOut().getSignal().getValue();
			auto q2r_cs = controlSys->invIencPos2.getOut().getSignal().getValue();
			auto q3_cs = controlSys->invIencPos3.getOut().getSignal().getValue();
			std::cout << "CS: axis0: " << q0_cs << ", axis1: " << q1_cs << ", axis2: " << q2r_cs << ", axis3: " << q3_cs << std::endl;
			count = 0;
		}
		else
			count++;
...
EncSafetyProperties::~EncSafetyProperties() {
	// nothing to do
}